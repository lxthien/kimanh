    ...
    public function getItemUrl(MenuItem $item)
    {
        switch ($item->getType()) {
            ...
            case MenuItem::TYPE_NEWS:
                if ($item->getTargetId()) {
                    $news = $this->em->getRepository('AppBundle:News')->find($item->getTargetId());
                    if ($news) {
            ...
            case MenuItem::TYPE_CATEGORY:
                if ($item->getTargetId()) {
                    $category = $this->em->getRepository('AppBundle:NewsCategory')->find($item->getTargetId());
                    if ($category) {
                        $url = $category->getUrl();
            ...
            case MenuItem::TYPE_PAGE:
                if ($item->getTargetId()) {
                    $page = $this->em->getRepository('AppBundle:News')->findOneBy([
                        'id' => $item->getTargetId(),
                        'postType' => 'page',
                    ]);
                    if ($page) {
