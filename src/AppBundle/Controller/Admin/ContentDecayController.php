...
class ContentDecayController extends Controller
{
    ...
    public function indexAction(Request $request, ContentDecayReporter $reporter)
    {
        ...
        $qb = $em->getRepository(News::class)->createQueryBuilder('n')
            ...
            ->andWhere('n.ignoreDecay = :ignoreDecay')
            ->setParameter('postType', 'post')
            ->setParameter('enable', true)
            ->setParameter('ignoreDecay', false)
        ...
        if ($filters['indexable'] === '1') {
            $qb->andWhere('n.robots NOT LIKE :noindex OR n.robots IS NULL')
    ...
    /**
     * @Route("/{id}/ignore", name="admin_content_decay_ignore", requirements={"id"="\d+"})
     * @Method("POST")
     */
    public function ignoreAction(Request $request, News $post)
    {
        if (!$this->isCsrfTokenValid('ignore_decay_' . $post->getId(), $request->request->get('_token'))) {
            return $this->json(['success' => false, 'message' => 'Invalid CSRF token.'], 403);
        }

        $post->setIgnoreDecay(true);
